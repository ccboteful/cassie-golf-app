...
export function CourseCard({ course }: { course: Course }) {
  return (
    <Link
      ...
      className="group overflow-hidden rounded-2xl border border-emerald-200/80 bg-white shadow-sm transition-all hover:-translate-y-0.5 hover:shadow-lg dark:border-emerald-900 dark:bg-emerald-950/30"
    >
      {course.imageUrl ? (
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-4 gap-2 rounded-xl bg-emerald-50 p-3 text-center dark:bg-emerald-900/30">
          ...
          <div>
            ...
            <p className="font-semibold text-zinc-900 dark:text-zinc-100">{metricValue(course.rating)}</p>
          </div>
          <div>
            <p className="text-[11px] uppercase text-zinc-500 dark:text-zinc-300">Slope</p>
            <p className="font-semibold text-zinc-900 dark:text-zinc-100">{metricValue(course.slope)}</p>
          </div>
          <div>
